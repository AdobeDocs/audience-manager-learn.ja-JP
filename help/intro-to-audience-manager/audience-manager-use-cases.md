---
title: Audience Managerのユースケース
description: この動画では、Audience Managerの一般的なユースケースを4つ紹介し、それらに関連するベストプラクティスを紹介します。
feature: Overview
topics: null
activity: understand
doc-type: feature video
team: Technical Marketing
kt: 5145
thumbnail: 33975.jpg
role: User, Developer, Admin, Leader
level: Beginner
exl-id: b0ed3450-0a0b-4bc3-b579-b57bfea116d7
TQID: https://experienceleague.adobe.com/wfsbVSNiAsJDLNOdCOBQt4UIVhUzGIgz0yzUNSbDCLU
product_v2:
  - id: df80eeb1-8d72-467e-b0df-9d51c7d3a0a1
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
  - id: f8a45b24-4be7-4f1b-909b-60d06b483a20
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
source-git-commit: 3152e8fc51e0e06c90c17dce0aa203a27995e88d
workflow-type: tm+mt
source-wordcount: 61
ht-degree: 14%

---

# Audience Managerのユースケース

この動画では、Audience Managerの一般的なユースケースを4つ紹介し、それらに関連するベストプラクティスを紹介します。

>[!VIDEO](https://video.tv.adobe.com/v/37271/?captions=jpn&quality=12)

詳しくは、[&#x200B; ドキュメント &#x200B;](https://experienceleague.adobe.com/docs/audience-manager/user-guide/aam-home.html?lang=ja)を参照してください。
