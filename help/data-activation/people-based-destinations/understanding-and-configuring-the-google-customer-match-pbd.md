---
title: Google Customer Match People-Based Destinationの理解と設定
description: このビデオでは、Google Customer Match People Based Destinationの詳細とユースケースについて説明し、セグメントを作成して宛先にマッピングする方法を説明します。 また、Google Ad Consoleにオーディエンスがランディングする様子も表示されます。
feature: People-based Destinations
topics: null
activity: use
doc-type: feature video
team: Technical Marketing
thumbnail: 38552.jpg
kt: 6042
role: User
level: Beginner
exl-id: c333e02e-a585-49aa-b095-f2dcbbd258a8
TQID: https://experienceleague.adobe.com/tds40it5-xWTnd4vzIPGusz0scN7DiNXJjblCTnvOBc
product_v2:
  - id: df80eeb1-8d72-467e-b0df-9d51c7d3a0a1
feature_v2:
  - id: c814092e-2730-45e8-a12d-e084529f52cb
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
source-git-commit: 3152e8fc51e0e06c90c17dce0aa203a27995e88d
workflow-type: tm+mt
source-wordcount: 107
ht-degree: 0%

---

# Google Customer Match People-Based Destinationの理解と設定

このビデオでは、Google Customer Match People Based Destinationの詳細とユースケースについて説明し、セグメントを作成して宛先にマッピングする手順を説明します。 また、Google Ad Consoleにオーディエンスがランディングする様子も表示されます。

>[!VIDEO](https://video.tv.adobe.com/v/326463/?captions=jpn&quality=12&learn=on)
